...
if($mode == 'edit' ){
	...
	if($ordendata->fields["fecha_orden"] != '0000-00-00 00:00:00')
	...
	if($ordendata->fields["fecha_orden2"] != '0000-00-00 00:00:00')
	...
	if($ordendata->fields["fecha_prox"] != '0000-00-00 00:00:00')
		$fecha_prox	= date("d-m-Y",strtotime($ordendata->fields["fecha_prox"]));
	$prox_mantenimientos  = $ESObject->getmantenimientos($ordendata->fields["tipo_prox"]);
	...
	$display = ($ordendata->fields["forma_pago"] == 3) ? "inline" : "none";
	...
	$refdisplay = ($articuloscode != '') ? 'block' : 'none';
	$norefdisplay = ($articuloscode != '') ? 'none' : 'block';
    ...
    $Newstemplate->asigna_variables( array( "ORDENID" 			=> $ordenid, 
											...
											"PROX_MANTENIMIENTOS" => $prox_mantenimientos,
    ...
    $Newstemplate->asigna_variables( array( "FORM_ACTION" => $formaction, "MODE" => $mode, "TALLERESBOX" => $talleresbox, "NEWID" => $newid,
											"MINUTOS" => $minutes, "HORAS" => $hours, "ARTICULOS" => $articulos, "TECNICOS" => $tecnicosbox, "MANTENIMIENTOS" => $mantenimientosbox,
											"FORMAS_PAGO" => $pagosbox, "DIAS" => $dias, "DROPTALLERES" => $droptalleres,"DIR"=>$dir) );
}
